...
function showTasks() {
	if(localStorage.getItem('tasks') == null || localStorage.getItem('tasks').length == 0 || localStorage.getItem('tasks') == 'Success.') {
		document.getElementById('tasks').innerHTML = 'Nothing on your to-do list!<br>Tap (+) to set a new task.';
	} else {
		document.getElementById('tasks').innerHTML = '';
		let input = decodeURI(localStorage.getItem('tasks').substring(8).replace(/%2C/, ',')).split('\n');
		let i = 0;
